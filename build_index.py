...
from config import CELL_INDEX_BUDGET
from offline.multiview_index import (
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build Agentic TableRAG Multiview Index"
    )
    parser.add_argument(
        "--excel_dir", required=True, help="Directory containing Excel/CSV table files"
    )
    parser.add_argument(
        "--doc_dir", required=True, help="Directory containing JSON/text document files"
    )
    parser.add_argument(
        "--bge_dir", default="BAAI", help="Directory containing bge-m3 model"
    )
    parser.add_argument(
        "--save_path", required=True, help="Output path prefix (e.g. ./index/multiview)"
    )
    parser.add_argument(
        "--budget",
        type=int,
        default=CELL_INDEX_BUDGET,
    ...
    parser.add_argument(
        ...
        "pairs encoded per table, subject to the remaining "
        "global budget; does not guarantee coverage of "
        "every table.",
    )
    parser.add_argument(
        "--row_budget",
    ...
    parser.add_argument(
        "--input_order",
        default=None,
        help="JSON manifest fixing table and document file order",
    )
    parser.add_argument(
        "--device", default="auto", help="Device for embedding: auto|cpu|cuda|cuda:0"
    )
    ...
    index = MultiviewIndex(
        ...
        input_order_path=args.input_order,
    )
    index.build()
    index.save()
