"""
Train the learned four-class DistilBERT router.

Prepare disjoint router_train.json and router_valid.json question subsets
from HybridQA-train before generating sub-query labels. Do not use downstream
dev evaluation questions or their traces for router training or selection.

Run the teacher-distillation workflow from the repository root:

python -m tools.train_router distill \\
    --data_file $DATA_DIR/hybridqa/router_train.json \\
    --excel_dir $DATA_DIR/hybridqa/train_excel \\
    --backbone gemini --out_file labels/router_train.jsonl

python -m tools.train_router train \\
    --oracle_file labels/router_train.jsonl \\
    --output_dir ./models/atr_router_distilbert --epochs 5

python -m tools.train_router distill \\
    --data_file $DATA_DIR/hybridqa/router_valid.json \\
    --excel_dir $DATA_DIR/hybridqa/train_excel \\
    --backbone gemini --out_file labels/router_valid.jsonl

python -m tools.train_router eval \\
    --oracle_file labels/router_valid.jsonl \\
    --model_dir ./models/atr_router_distilbert

The eval command scores all supplied records; it does not split them.
Keep all sub-queries from each source question in one partition.

Label JSONL records include sub_query, expected_operator, required_modalities,
entity_mentions, has_schema, and oracle_route (TEXT / SQL / RETRIEVE / HYBRID).
See README.md for input formats and router training commands.
"""
...
# Relative route-cost offsets used only by the execution-oracle alternative.
# These are heuristic penalties, not measured end-to-end LLM call counts.
...
def featurise(record: Dict[str, Any], input_version: int = 2) -> str:
    from online.router import build_router_input

    if input_version == 1:
        # The legacy trainer used has_schema only, even if other metadata exists.
        return build_router_input(
            sub_query=record.get("sub_query", ""),
            expected_operator=record.get("expected_operator", "lookup"),
            required_modalities=record.get("required_modalities", "both"),
            entity_mentions=record.get("entity_mentions", []),
            need_global_table_view=False,
            uncertainty=0.0,
            schema={} if record.get("has_schema", False) else None,
            history_H=[],
            input_version=1,
        )
...
def _infer_operator(question: str) -> str:
    ...
    if any(
        w in q
        for w in ("greater than", "less than", "more than", "fewer than", "between")
    ):
        return "compare"
    return "lookup"
...
def _execute_route_for_oracle(
    ...
    from online.router import Route
...
def generate_true_oracle(
    ...
    """
    Execution-based oracle labels, an alternative to teacher distillation.
    ...
    """
    from config import CELL_TOP_K, SCHEMA_TOP_K
    from offline.multiview_index import MultiviewIndex
    from online.constrained_sql import ConstrainedSQLExecutor
    from online.decomposer import QueryDecomposer
    from online.value_linker import HybridValueLinker
    from online.verifier import EvidenceFusionVerifier
    ...
    with open(out_file, "w") as fout:
        for i, item in enumerate(data):
            ...
            schema = (
                index.restore_schema_via_mapping(table_chunks) if table_chunks else None
            )
            text_evidence = "\n\n".join(
            ...
            if correct_routes:
                oracle_route = min(
                    correct_routes, key=lambda r: route_results[r]["cost"]
                )
                oracle_correct = True
            else:
                # Soft label: cheapest route (will be flagged oracle_correct=False)
                oracle_route = min(
                    route_results, key=lambda r: route_results[r]["cost"]
                )
    ...
    n_correct = sum(
        1 for r in _load_oracle_records(out_file) if r.get("oracle_correct")
    )
    logger.info(
...
def relabel_soft_records(oracle_file: str, out_file: str) -> None:
    """
    Replace labels for oracle_correct=False records using
    ...
    keep their original labels. This heuristic relabeling is separate from
    the LLM teacher-distillation workflow and does not guarantee better labels.
    """
...
def load_inference_oracle(inference_log: str, out_file: str) -> None:
    ...
    Current logs are produced with ``online.main --emit_trace`` and store
        ...
        for line in fin:
            ...
            for decision in candidates:
                ...
                oracle_rec = {
                    ...
                    or decision.get("has_schema", rec.get("has_schema", False)),
                    "history_H": decision.get("history_H", rec.get("history_H", [])),
                    "oracle_route": route,
                    "oracle_correct": True,
...
def train(
    ...
    Checkpoint selection uses an internal random record-level validation split.
    Records from the same source question are not grouped by this split; use
    a separate question-level holdout for reporting router agreement.
    ...
    from online.router import ROUTER_INPUT_VERSION

    model.config.router_input_version = ROUTER_INPUT_VERSION
    model.config.router_max_length = max_len
    ...
    for epoch in range(1, epochs + 1):
        ...
        logger.info(
            f"Epoch {epoch}/{epochs}  loss={avg_loss:.4f}  val_acc={val_acc:.4f}"
        )
...
def evaluate(oracle_file: str, model_dir: str, max_len: int = 256) -> float:
    ...
    from online.router import validate_router_checkpoint

    model_config = AutoConfig.from_pretrained(model_dir)
    trained_max_len = validate_router_checkpoint(model_config)
    if max_len != trained_max_len:
        raise ValueError(
            f"Evaluation max_len={max_len} differs from checkpoint length {trained_max_len}"
        )
    records = _load_oracle_records(oracle_file)
    ...
    confusion: Dict[str, Dict[str, int]] = {
        r: {r2: 0 for r2 in LABEL2ID} for r in LABEL2ID
    }

    with torch.no_grad():
        for rec in records:
            text = featurise(
                rec, input_version=getattr(model_config, "router_input_version", 1)
            )
...
#
# Optional extended trainer; the README's standard workflow uses train().
# The train_v4 command name is not a checkpoint router_input_version.
#
...
#   --cost_weight λ       : add λ × Σᵣ P(r) · cost(r) to loss.
...
def _meta_features_vec(rec: Dict[str, Any]):
    ...
    uncertainty = float(
        rec.get("decomposer_uncertainty", rec.get("uncertainty", 0.0)) or 0.0
    )
...
def _group_by_qid(records: List[Dict]) -> List[Dict]:
    """Multi-label: collapse multiple records per question into one record.
...
def train_v4(
    ...
    """Optional multi-label, meta-feature, cost-aware and curriculum training.

    Dataset exclusion supports leave-one-dataset-out experiments. These options
    are distinct from the standard teacher-distillation workflow in README.md.
    """
    try:
        import torch
        import torch.nn as nn
    ...
    if multi_label:
        ...
        logger.info(
            f"[multi_label] grouped {len(all_records)} records → {len(records)} unique Q"
        )
    else:
        records = all_records
    ...
    if multi_label:
        ...
        if focal_loss:
            ...
            def compute_loss(logits, batch):
                ce = F.cross_entropy(
                    logits, batch["labels"], weight=class_weights, reduction="none"
                )
                pt = torch.exp(-ce)
    ...
    if curriculum and multi_label:
        stages = [
            (
                "stage1_single_correct",
                max(1, epochs // 3),
                lambda r: sum(r["correct_routes_multi_hot"]) == 1,
                0.0,
            ),
            ("stage2_multi_correct", max(1, epochs // 3), lambda r: True, 0.0),
            (
                "stage3_cost_penalty",
                epochs - 2 * max(1, epochs // 3),
                lambda r: True,
                cost_weight,
            ),
        ]
    ...
    for stage_name, stage_epochs, stage_filter, stage_cost in stages:
        ...
        for ep in range(1, stage_epochs + 1):
            ...
            if metric >= best_val_metric:
                ...
                if use_meta_features:
                    torch.save(
                        model.state_dict(), os.path.join(output_dir, "router_v4.pt")
                    )
                    ...
                    tokenizer.save_pretrained(output_dir)
                logger.info(
                    f"  → saved best model (metric={metric:.4f}) to {output_dir}"
                )
...
def generate_llm_distill(
    ...
    """
    Generate hard route labels from the LLM-router teacher.

    For each (optionally sampled) question:
      1. Decompose → first sub-query q_t  (decomposer retries may apply)
      2. LLMRouter.route(q_t, H)           (up to 4 LLM calls)
    and record the initial choice plus teacher re-selections under cumulative
    failed-route histories. Routes are added to the history without executing
    or verifying their answers; these are simulated failure histories, not
    observed execution failures. When ``excel_dir`` is supplied, column names,
    inferred types and sample values from that table are serialized as schema.
    ...
    from online.decomposer import QueryDecomposer
    from online.router import LLMRouter
    ...
    logger.info(
        f"LLM-distill over {len(data)} questions (backbone={backbone}, workers={max_workers})"
    )
    ...
    with open(out_file, "w") as fout, ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for done, fut in enumerate(as_completed(futures), 1):
            ...
            if done % 200 == 0:
                logger.info(
                    f"  [{done}/{len(data)}] written={written} skipped={skipped} {label_counts}"
                )
...
def build_parser() -> argparse.ArgumentParser:
    """Define label-generation, training, and evaluation subcommands."""
    ...
    gen = sub.add_parser(
        "generate", help="Pseudo-oracle labels via HeuristicRouter (no LLM)"
    )
    ...
    # oracle (execution-based alternative)
    ora = sub.add_parser(
        "oracle",
        help="Alternative execution-oracle labels: run routes and select a correct route by cost",
    )
    ...
    ora.add_argument(
        "--bge_dir", required=True, help="Directory containing bge-m3 model"
    )
    ora.add_argument(
        "--backbone", default="gemini", help="LLM backbone key from config_mapping"
    )
    ...
    dst.add_argument(
        "--data_file",
        required=True,
        help="QA dataset JSON/JSONL (e.g. HybridQA train.json)",
    )
    dst.add_argument(
        "--backbone", default="gemini", help="LLM backbone key from config_mapping"
    )
    ...
    dst.add_argument(
        ...
        type=int,
        default=None,
        help="Sample this many questions (seeded)",
    )
    ...
    rl = sub.add_parser(
        "relabel",
        help="Replace oracle_correct=False labels using heuristic routing",
    )
    rl.add_argument(
        "--oracle_file",
        required=True,
        help="Input oracle JSONL (with oracle_correct field)",
    )
    ...
    tr4.add_argument(
        "--multi_label",
        action="store_true",
        help="Sigmoid+BCE multi-label classification "
    ...
    tr4.add_argument(
        "--use_meta_features",
        action="store_true",
        help="Concatenate operator/modality/entity_count/has_schema/"
    ...
    tr4.add_argument(
        ...
        type=float,
        default=0.0,
        help="Weight for the expected-cost penalty term in loss",
    )
    tr4.add_argument(
        "--curriculum",
        action="store_true",
        help="Three-stage curriculum (single→multi→cost). Requires --multi_label",
    )
    tr4.add_argument(
        "--exclude_dataset",
        type=str,
        ...
        help="Exclude this dataset from training (leave-one-dataset-out evaluation)",
    )
...
def main(argv: Optional[List[str]] = None) -> None:
    """Dispatch the selected router workflow."""
    _setup_logging()
