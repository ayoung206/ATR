...
because the FAISS indices in :mod:`offline.multiview_index` are inner-product
...
class Embedder:
    ...
    def encode(self, texts: Sequence[str]) -> np.ndarray:
        ...
        # CPU float32 conversion can be a no-op: the CLS slice then keeps the
        # entire [batch, tokens, hidden] storage alive in every cached batch.
        # Own only the pooled vectors, without changing their values.
        return cls_vectors.float().cpu().numpy().copy()
