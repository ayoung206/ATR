...
def _failed_route_names(
    ...
    """Paper H excludes rejected primitives across the entire question.

    Keep sub_query in the call signature for compatibility, not as a filter.
    The agent creates a fresh H for each new original question.
    """
    failed = set()
    for item in history_H or []:
        if not isinstance(item, dict):
            ...
            route = item.get("effective_route") or item.get("route")
        if isinstance(route, Route):
            route = route.value
        if route:
            failed.add(str(route).upper())
    return failed
...
def build_router_input(
    ...
    """Serialize the checkpoint's input contract, without changing its weights."""
    entities = ", ".join(entity_mentions or []) or "none"
    if input_version == 1:
        # Exact legacy training/inference representation. H is applied as an
        # output mask, not injected into this student's text input.
    ...
    if input_version != 2:
        raise ValueError(f"Unsupported router input version: {input_version}")
...
ROUTER_INPUT_VERSION = 2
ROUTER_MAX_LENGTH = 256
ROUTER_LABELS = {0: "TEXT", 1: "SQL", 2: "RETRIEVE", 3: "HYBRID"}


def validate_router_checkpoint(config: Any) -> int:
    """Reject incompatible students before loading tokenizer or model weights."""
    version = getattr(config, "router_input_version", 1)
    if type(version) is not int or version not in (1, ROUTER_INPUT_VERSION):
        raise ValueError(
            f"Incompatible learned router: router_input_version={version!r}; "
            "supported versions are 1 (legacy) and 2 (schema/history inputs). "
            "Do not change version metadata without checking the training format."
        )
    if (
        getattr(config, "model_type", None) != "distilbert"
        or getattr(config, "num_labels", None) != 4
        or getattr(config, "id2label", None) != ROUTER_LABELS
        or getattr(config, "label2id", None) != {v: k for k, v in ROUTER_LABELS.items()}
    ):
        raise ValueError(
            "Incompatible learned router architecture or TEXT/SQL/RETRIEVE/HYBRID label mapping"
        )
    max_length = getattr(config, "router_max_length", ROUTER_MAX_LENGTH)
    if (
        type(max_length) is not int
        or max_length <= 0
        or max_length > getattr(config, "max_position_embeddings", 512)
    ):
        raise ValueError("Invalid learned router router_max_length")
    return max_length
...
class LearnedRouter(BaseRouter):
    ...
    Version 1 preserves legacy experiment inputs and masks failed routes.
    Version 2 additionally encodes the detailed schema and H in the input.
    ...
    def __init__(self, model_path: str, device: str = "cpu") -> None:
        ...
        self.max_length = validate_router_checkpoint(model_config)
        self.input_version = getattr(model_config, "router_input_version", 1)
        if self.input_version == 1:
            logger.info(
                "LearnedRouter uses legacy input format; H is applied as an output mask"
            )
    ...
    def featurise(sub_query: str, meta: Any, schema: Optional[Dict]) -> str:
        ...
        return build_router_input(
            ...
            required_modalities=meta.required_modalities or "both",
            entity_mentions=meta.entity_mentions or [],
            need_global_table_view=bool(getattr(meta, "need_global_table_view", False)),
            uncertainty=getattr(meta, "uncertainty", 0.0),
            schema=schema,
            history_H=[],
        )
    ...
    def route(
        ...
        # retries call ``reselect``; v1 masks H, while v2 also encodes H.
    ...
    def escalate(
        ...
        failed.update(_failed_route_names(history_H, ""))
    ...
    def reselect(
        ...
        """Re-run with updated H: output mask for v1, also input features for v2."""
        return self.route(sub_query, meta, schema, history_H)
...
def build_router(
    ...
    if router_type == "fixed":
        ...
        return FixedRouter(
            Route(force_route.upper()), escalate_chain=fixed_escalate_chain
        )
    raise ValueError(
        f"Unknown router_type '{router_type}'. Choose: heuristic | llm | learned | fixed"
    )
