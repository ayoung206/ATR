...
from prompt import VALUE_LINKER_PROMPT
...
def _prior_value_linker_failures(
    ...
    """Count linking failures in the question-wide H, across sub-queries."""
    count = 0
    for item in history_H or []:
...
class HybridValueLinker:
    ...
    def link(
        ...
        Args:
            ...
            current_sub_query: retained for caller compatibility; H is question-wide

        Returns:
            List of LinkedValue objects.
        """
        prior_vl_failures = _prior_value_linker_failures(history_H, current_sub_query)
        ...
        # reroute. Earlier sub-queries in this question also count; failures
        # that did not invoke linking do not consume this fallback ladder.
    ...
    def _link_one(
        ...
        schema_by_col = {
            ...
            and isinstance(c.get("col_name"), str)
        ...
        allowed_pairs = {
            (
                c.get("table_id") or c.get("table_name") or c.get("source"),
                c.get("col_name"),
            )
            for c in schema_columns
            if isinstance(c, dict)
        ...
        for candidate in candidates:
            ...
            if scoped and (not table or (table, col) not in allowed_pairs):
                continue
            ...
            if isinstance(val, float) and not math.isfinite(val):
                continue
            val = str(val)
            if not val.strip() or val == NO_MATCH or (table, col, val) in seen_pairs:
                continue
            seen_pairs.add((table, col, val))
            valid_candidates.append({**candidate, "value": val, "table_id": table})
        candidates = valid_candidates
        if not candidates:
            logger.debug(f"No candidates for entity '{entity}'")
            return self._apply_fallback(
                entity, "", [], min_fallback, allow_text_fallback
            )
        ...
        for c in candidates:
            col_counts[c.get("col_name", "")] = (
                col_counts.get(c.get("col_name", ""), 0) + 1
            )
        ...
        return LinkedValue(
            ...
            confidence,
            fallback_level=0,
            table_id=matches[0].get("table_id"),
        )
    ...
    def _apply_fallback(
        ...
        if min_fallback <= 2 or not allow_text_fallback:
            ...
            for candidate in candidates:
                ...
                if (
                    entity_norm
                    and val_norm
                    and (entity_norm in val_norm or val_norm in entity_norm)
                ):
                    if (
                        len(
                            {
                                (c.get("table_id"), c["col_name"])
                                for c in candidates
                                if c["value"] == val
                            }
                        )
        ...
        if not allow_text_fallback:
            logger.debug(
                ...
                "match for '%s', dropping this entity's binding",
                entity,
            )
            return LinkedValue(entity, col_name, None, 0.0, fallback_level=1)
