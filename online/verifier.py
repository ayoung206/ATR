"""Evidence fusion and verifier: synthesises a candidate answer and validates it (0 / 0.5 / 1); rejection triggers re-selection by the active router."""
...
from online.parsing import parse_json_object as _parse_json
from prompt import (
...
def _is_grounded(answer: str, evidence: str) -> tuple:
    """Check whether an answer is grounded in the supplied evidence.
...
def _extract_sql_scalar(sql_result: str) -> Optional[str]:
    ...
    Handles common SQL result formats:
      ...
      - plain string scalar
    Returns None if result is SQL NULL, multi-row, multi-column, or ambiguous.
    A JSON null must not become the answer string "None"; actual strings with
    that spelling and numeric zero remain valid scalar data.
    """
    ...
    if (
        ...
        and len(parsed) == 1
        and not isinstance(parsed[0], (list, dict))
    ):
        return None if parsed[0] is None else str(parsed[0]).strip()
...
def _validated_verdict(value: Any) -> float:
    """Malformed, non-finite or out-of-domain judgments are insufficient."""
    ...
    try:
        ...
        return VERDICT_INSUFFICIENT
    if not math.isfinite(verdict) or verdict not in (
        VERDICT_CONFLICT,
...
class EvidenceFusionVerifier:
    ...
    def fuse(
        ...
        scalar return without markdown cross-validation.
        """
    ...
    def answer_from_retrieval(
        ...
        """RETRIEVE route: synthesise from schema, grounded cells, and row context.

        The caller supplies retrieved rows as table_context so the LLM can
        read answer columns beyond the cells selected for entity linking.
        """
    ...
    def verify(
        ...
        verdict = _validated_verdict(raw_verdict)

        reason = parsed.get("reason", "")
        logger.debug(f"Verifier verdict={verdict}, reason={reason}")

        # Check answer grounding before accepting an LLM support verdict.
    ...
    def is_rejected(self, verdict: float) -> bool:
        """Only a valid support judgment may authorize an answer."""
        return _validated_verdict(verdict) != VERDICT_SUPPORT
