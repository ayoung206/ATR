"""ATR online orchestration: decompose, route, execute, verify, and synthesize."""
...
from clients.chat_utils import get_chat_result, init_logger
from config import (
    CELL_TOP_K,
    MAX_ITER,
...
from online.decomposer import QueryDecomposer, SubQuery
from online.router import Route, build_router
...
def _build_schema_preview(table_chunks: List[Dict], max_rows: int = 3) -> str:
    """Build a compact "column headers + sample rows" hint for the decomposer.
...
def _row_hits_to_markdown(hits: List[Dict], max_rows: int = ROW_TOP_K) -> str:
    """Render View 4 row-component hits as a compact markdown listing.
...
def _table_chunks_to_markdown(chunks: List[Dict], max_chars: int = 4000) -> str:
    """Concatenate table-type chunk text for verifier fusion.
...
def _cell_entries_to_text(linked_values: Any) -> str:
    ...
    for lv in linked_values:
        if lv.is_matched:
            column = f"{lv.table_id}.{lv.column}" if lv.table_id else lv.column
            lines.append(f"Entity '{lv.entity}' → {column} = '{lv.matched_value}'")
...
def _arithmetic_hint(text: str) -> str:
    """Formula hint based on detected arithmetic intent."""
    t = (text or "").lower()
    if "percentage change" in t or "percent change" in t or "% change" in t:
        return (
            "Compute (new_value - old_value) / old_value * 100. "
            "Output as a percent value with sign (e.g., '-12.14 percent')."
        )
    if any(k in t for k in ("how many", "number of", "count of", "count the")):
        return (
            "Use SELECT COUNT(*) FROM ... WHERE <condition> with the threshold "
            "applied strictly. Return a single integer scalar."
        )
    if any(
        k in t
        for k in ("change", "difference", "increase", "decrease", "growth", "decline")
    ):
        return (
            "Compute new_value - old_value. Preserve unit (e.g., million / thousand / percent). "
    ...
    return (
        "Perform the requested computation in SQL. "
        "Return the computed scalar with unit, NOT raw rows."
    )


def _column_type_hint(schema_entries: List[Dict]) -> str:
    """Provide a hint about column types, when VARCHAR holds numeric strings, suggest REPLACE/CAST."""
    suggestions = []
...
def _augment_sql_query(
    ...
    """Wrap sub_query with NL2SQL-friendly hints using arithmetic and column-type context.
...
class AgenticTableRAGAgent:
    ...
    def __init__(
        ...
        # Optional context and fusion settings:
        ...
        #                        fast-path; skips markdown cross-validation
        ...
        if decomposer_backbone and decomposer_backbone != backbone:
            ...
            logger.info(
                "Decomposer backbone overridden: %s (pipeline backbone: %s)",
                decomposer_backbone,
                backbone,
            )
        else:
        ...
        if verifier_backbone and verifier_backbone != backbone:
            ...
            logger.info(
                "Verifier (verdict) backbone overridden: %s (pipeline backbone: %s)",
                verifier_backbone,
                backbone,
            )
        else:
    ...
    def run_single(
        ...
        Returns:
            final answer string
        """
        # Each question starts with empty answer and failure histories.
        ...
        # Expand caller-supplied table names to backend lookup variants.
        ...
        sql_executor = ConstrainedSQLExecutor(
            ...
            table_schemas=getattr(self.index, "table_schemas", {}),
        )
        table_id_hint = raw_table_names[0] if raw_table_names else ""

        consecutive_failures = 0  # tracks consecutive "not found" steps

        # Pre-retrieve table context for the optional decomposer schema preview.
        ...
        for t in range(1, effective_max_iter + 1):
            ...
            # Decompose with prior answers and failure context
            ...
            logger.info(
                f"[{question_id}] sub_query='{sub_q.sub_query}'  "
            ...
            # Retrieve context and restore schema
            ...
            # §3.6 Stop Controller
            # Stop only when the verifier supports the current answer and the
            # decomposer reports uncertainty below the configured threshold.
            ...
            trace.update(
                {
                    ...
                    "n_escalations": sum(
                        1 for decision in verifier_decisions if not decision["accepted"]
                    ),
    ...
    def _synthesize_final(
        ...
        from prompt import FINAL_SYNTHESIS_PROMPT
    ...
    def _execute_and_verify(
        ...
        """
        Execute the selected primitive and re-route after failed verification.
        ...
        for attempt in range(max_attempts):
            ...
            # Verify the intermediate answer
                ...
                decisions.append(
                    {
                        ...
                        "expected_operator": getattr(
                            sub_q, "expected_operator", "lookup"
                        ),
                        "required_modalities": getattr(
                            sub_q, "required_modalities", "both"
                        ),
            ...
            if self.no_escalation:
                ...
                logger.info(
                    f"[{question_id}] no_escalation=True → returning first-route answer"
                )
                break
            ...
            if new_route == effective_route:
                # Stop when re-selection offers no different primitive.
                break
            route = new_route
    ...
    def _execute_route(
        ...
        # SQL
        if route == Route.SQL:
            # Schema-only retrieval (BGE+FAISS, no cell index, no value
            # linker LLM). Calling `retrieve_schema` directly avoids the per-entity
            # cell loop so SQL-route latency stays low.
            try:
                C_sql = self.index.retrieve_schema(sub_q.sub_query, top_k=SCHEMA_TOP_K)
            except Exception as exc:
        ...
        # RETRIEVE
        # Pass View 4 row-component (RowIndex) rows to the synthesis
        ...
        # HYBRID
        # Add arithmetic and column-type hints to the SQL sub-query.
        # passed to ConstrainedSQLExecutor. The column constraint is already
...
class BatchRunner:
    ...
    def run(
        ...
        if rerun and os.path.exists(save_file):
            ...
            logger.info(
                f"Resume: {len(done_questions)} questions already done in {save_file}"
            )
        else:
            done_questions = set()
...
def build_parser() -> argparse.ArgumentParser:
    """Define inference arguments independently from model initialization."""
    ...
    parser.add_argument(
        ...
        "(default cuda; DistilBERT has 66M parameters). "
        "Use cpu when CUDA is unavailable.",
    )
    # Schema preview and markdown fusion are opt-in ablations.
    # Keep them disabled when comparing against runs made with the defaults.
    parser.add_argument(
        "--use_schema_preview",
        action="store_true",
        help="Pre-retrieve once; inject column headers + "
        "sample rows into decomposer prompt. Default OFF.",
    )
    parser.add_argument(
        "--use_markdown_fusion",
        action="store_true",
        help="Conditional fast-path + EVIDENCE_FUSION_PROMPT "
...
def main(argv: Optional[List[str]] = None) -> None:
    """Load the index and router, then run batch inference."""
    ...
    if args.save_file_path:
        ...
        args.save_file_path = str(
            ...
            / "output"
            / f"{args.router_type}_results_{_ts}.jsonl"
        )
