...
from clients.tool_utils import Embedder, Reranker, excel_to_markdown
from offline.reranking import CrossEncoderCandidateReranker
...
def _validate_index_payload(save_path: str, payload: Dict[str, Any]) -> None:
    ...
    if not isinstance(build_config, dict):
        ...
        violations.append(
            "index_build_config.document_chunks does not match document_chunk_config"
        )
    ...
    if all(isinstance(items, list) for items in (doc_chunks, doc_sources, doc_types)):
        if not (len(doc_chunks) == len(doc_sources) == len(doc_types)):
            violations.append("doc_chunks, doc_source, and doc_type lengths must match")
    ...
    if not isinstance(build_config.get("embedding_model"), str) or not build_config.get(
        ...
        violations.append(
            "index_build_config.embedding_model must be a non-empty string"
        )
    ...
    if isinstance(table_schemas, dict) and table_schemas and not cell_entries:
        violations.append("table schemas exist but the required cell index is empty")
    ...
    for label, entries, quota in (
        ...
        if not isinstance(entries, list) or not isinstance(quota, int) or quota <= 0:
            continue
        counts = Counter(
            entry.get("table_id") for entry in entries if isinstance(entry, dict)
        )
...
class SchemaIndex:
    ...
    def __init__(
        ...
        self.entries: List[
            Dict[str, Any]
        ] = []  # [{table_id, col_name, dtype, examples}]
    ...
    def retrieve(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        _, indices = self._index.search(q_emb, search_k)
        candidates = [self.entries[i] for i in indices[0] if 0 <= i < len(self.entries)]
...
class CellIndex:
    """
    View 4: cell and row retrieval (§3.2).
    ...
    def __init__(
        ...
        Args:
            ...
            per_table_quota:  maximum (column, value) pairs encoded per table,
                              subject to the remaining global budget. Tables
                              processed after budget exhaustion get no cells;
                              this is not a balanced allocation across tables.
        """
        self.embedder = embedder
        self.reranker = reranker
        self.budget = budget
    ...
    def add_table(self, table_id: str, df: pd.DataFrame) -> None:
        ...
        """
        # pandas 3 infers StringDtype ("str") instead of object for text.
        # Keep the existing object semantics while also accepting nullable
        # string / Arrow string columns. The ingestion loader uses dtype=str.
        cat_cols = [
            c
            for c in df.columns
...
class RowIndex:
    """
    View 4 row component: RowIndex (§3.2).
            ...
            self.entries.append(
                {
                    ...
                    "row_text": row_text,
                    "row_dict": {
                        c: (str(row[c]) if pd.notna(row[c]) else "") for c in df.columns
                    },
                }
            )
...
class MultiviewIndex:
    ...
    After `build()`:
      ...
      self.row_index        → View 4 row component
    ...
    def __init__(
        ...
        max_row_chars: Optional[int] = None,
        input_order_path: Optional[str] = None,
    ) -> None:
        ...
        if input_order_path:
            with open(input_order_path, encoding="utf-8") as stream:
                self.input_order = json.load(stream)
            if not isinstance(self.input_order, dict):
                raise ValueError("Input order must be a JSON object")
            for key, directory in (("tables", excel_dir), ("documents", doc_dir)):
                self._source_files(directory, key)

        self.embedder = Embedder(
            bge_model_path, device=device, require_cuda=require_cuda
        )
        self.reranker = (
            _load_reranker(
                reranker_model_path,
        ...
        # View 4 row component: per-row embeddings for RETRIEVE.
    ...
    def build(self) -> None:
        ...
        """
        self._ingest_tables()
        self._require_cell_entries()
        self._ingest_text_docs()
        self.doc_retriever.build()
        self.schema_index.build()
        self.cell_index.build()
        self.row_index.build()  # View 4 row component
        logger.info("MultiviewIndex build complete.")

    def _source_files(self, directory: str, kind: str) -> List[str]:
        names = [
            name
            for name in os.listdir(directory)
            if os.path.isfile(os.path.join(directory, name))
            and (kind != "tables" or name.lower().endswith((".xlsx", ".csv")))
        ]
        order = getattr(self, "input_order", None)
        if order is None:
            return names
        requested = order.get(kind)
        if (
            not isinstance(requested, list)
            or not all(isinstance(name, str) for name in requested)
            or len(requested) != len(set(requested))
            or set(requested) != set(names)
        ):
            raise ValueError(f"Input order does not match {kind} files")
        return requested

    def _ingest_tables(self) -> None:
        if not os.path.isdir(self.excel_dir):
            return
        for fname in self._source_files(self.excel_dir, "tables"):
            ...
            # View 4 (row component): row entries (per-row text for RETRIEVE)
            self.row_index.add_table(table_id, df)

    def _ingest_text_docs(self) -> None:
        if not os.path.isdir(self.doc_dir):
            return
        for fname in self._source_files(self.doc_dir, "documents"):
    ...
    def retrieve_rows(
        ...
        """Row-level retrieval (View 4 row component).
    ...
    def schema_cell_retrieval(
        ...
        col_names = list(dict.fromkeys(entry["col_name"] for entry in C))
        allowed_pairs = {
            (c.get("table_id") or c.get("table_name") or c.get("source"), c["col_name"])
            for c in C
        }
        scoped = any(table for table, _ in allowed_pairs)
        if cell_top_k < 0:
            raise ValueError("cell_top_k must be non-negative")

        V_raw: Dict[str, List[Dict[str, Any]]] = {}
        for entity in entity_mentions:
            if cell_top_k == 0 or not col_names:
                V_raw[entity] = []
                continue
            candidates = []
            for col in col_names:
                candidates.extend(
                    self.retrieve_cells(entity, column=col, top_k=cell_top_k)
                )
            # Column-conditioned retrieval scores are not comparable across
            # queries. Merge first, then rank the whole pool against entity.
            seen = set()
            deduped = []
            for c in candidates:
                if c["col_name"] not in col_names:
                    continue
                table = c.get("table_id") or c.get("table_name") or c.get("source")
                if scoped and (
                    not table or (table, c["col_name"]) not in allowed_pairs
                ):
                    continue
                key = (table, c["col_name"], c["value"])
                if key not in seen:
                    seen.add(key)
                    deduped.append(c)
            if not deduped:
                V_raw[entity] = []
                continue
            # Use a canonical tie order so schema ordering cannot decide which
            # equal-scoring pair survives. Preserve the original index entries.
            deduped.sort(
                key=lambda c: (
                    c["col_name"],
                    c["value"],
                    c.get("table_id") or c.get("table_name") or c.get("source") or "",
                )
            )
            passages = [_cell_entry_to_text(c) for c in deduped]
            if self.cell_index.reranker:
                V_raw[entity] = self.cell_index.reranker.select(
                    entity, deduped, passages, cell_top_k
                )
            else:
                # The no-reranker ablation still needs a common dense query;
                # concatenation order is not a retrieval score.
                query_vector = self.cell_index.embedder.encode([entity])[0]
                vectors = self.cell_index._embed_in_batches(passages)
                scores = vectors @ query_vector
                order = sorted(range(len(deduped)), key=lambda i: -float(scores[i]))
                V_raw[entity] = [deduped[i] for i in order[:cell_top_k]]

        return C, V_raw

    # persistence
    def _require_cell_entries(self) -> None:
        if self.table_schemas and not self.cell_index.entries:
            raise ValueError(
                "table schemas exist but the required cell index is empty; "
                "check input values, pandas string dtypes and cell quotas "
                "before building or saving a Full ATR index"
            )
    ...
    def save(self) -> None:
        ...
        if self.cell_index._index is not None:
            faiss.write_index(self.cell_index._index, self.save_path + ".cell.faiss")
        if self.row_index._index is not None:
            faiss.write_index(self.row_index._index, self.save_path + ".row.faiss")
    ...
    def load(
        ...
        reranker = None
        if enable_reranker:
            resolved_reranker_path = reranker_model_path or _default_reranker_path(
                bge_model_path
            )
        ...
        instance.input_order = payload["index_build_config"].get("input_order")
